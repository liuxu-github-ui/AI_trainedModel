...
{
    public class CATVSDOG : BaseImagePredictor<sbyte>
    {
        ...
        float[,] outputs0 = new float[2, 4]; // [top, left, bottom, right] * 10
        float[] outputs1 = new float[2]; // Classes
        float[] outputs2 = new float[10]; // Scores
        Result[] results = new Result[2];

        public CATVSDOG(string modelPath) : base(modelPath, true)
        {
        }
